...
**Relationship to footguns.md:** `ai-docs/footguns/` is the central index and source of truth. Footguns mapped to a specific directory are **propagated** (not moved) as one-line summaries into local instruction files.
...
| **Learning loop** | `ai-docs/footguns/` (cross-domain coupling with file:line evidence), `ai-docs/lessons/` (what worked/failed) | Updated after every task |

**Cold path (ai-docs/coding-standards/):** Project-specific coding guidelines organized by domain. Loaded on demand when the task matches -- frontend.md for frontend work, backend.md for backend work, etc. Router at `ai-docs/README.md` tells agents what to load. Keeps the hot path under 120 lines by moving domain details to dedicated files.

**Create on first use:** Two artifacts materialise when first needed, not pre-created empty: agent profiles directory (e.g., `.claude/profiles/` or `.gemini/profiles/`, create when meaningful role separation exists), and `ai-docs/decisions/` (create when there's a real architectural decision worth recording). All other artifacts are created during initial setup.
...
| Domain instructions | ai-docs/coding-standards/ | ai-docs/coding-standards/ | .cursor/rules/ | .github/instructions/ (bridges to ai/) | ai-docs/coding-standards/ |
| Evals | ai-docs/evals/ | ai-docs/evals/ | ai-docs/evals/ | ai-docs/evals/ | ai-docs/evals/ |
| Deny mechanism | permissions.deny array | .codex/rules/deny-dangerous.star (execpolicy) | - | - | permissions.deny array |

Setup guides: see `workflow/setup/setup-claude.md`, `workflow/setup/setup-gemini.md`, `workflow/setup/setup-codex.md`, and `workflow/setup/setup-copilot.md`.
