...
2. **Check `ai-docs/footguns/`** - these are real traps in this codebase with file:line evidence. Read before making changes.
3. **Check `ai-docs/lessons/`** - behavioral mistakes that have happened before. Don't repeat them.
...
- **Behavioral mistake** (you did something wrong) → add to `ai-docs/lessons/`
- **Architectural trap** (the code has a hidden coupling) → add to `ai-docs/footguns/` with `file:line` evidence
- **Technical decision** (significant choice with trade-offs) → add to `ai-docs/decisions/ADR-NNN-*.md`
