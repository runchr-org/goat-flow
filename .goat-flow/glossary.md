...
| Ask First | The middle autonomy tier requiring the agent to pause and confirm with the human before touching high-risk boundaries. | `workflow/setup/shared/execution-loop.md` | Micro-checklist |
| Autonomy Tiers | Three-level permission system (Always / Ask First / Never) controlling what the agent can do without human approval. | `workflow/setup/shared/execution-loop.md` | -- |
| Blast Radius | The declared maximum scope of files and systems a task is allowed to touch before the agent must stop and re-scope. | `workflow/setup/shared/execution-loop.md` | -- |
...
| Cold Path | Documentation loaded on demand (coding standards, playbooks) rather than every session. | `workflow/setup/shared/system-overview.md` | -- |
| Definition of Done | Six explicit gates that must all pass before a task is considered complete. | `workflow/setup/shared/execution-loop.md` | DoD |
...
| Execution Loop | The six-step agent workflow: READ, CLASSIFY, SCOPE, ACT, VERIFY, LOG. | `workflow/setup/shared/execution-loop.md` | Default loop |
| Footgun | A documented architectural trap with `file:line` evidence stored in category bucket files under `ai-docs/footguns/`. | `workflow/setup/shared/phase-1.md` | Architectural landmine |
...
| Hot Path | Instruction content loaded every session (CLAUDE.md, local instruction files) with a strict line budget. | `workflow/setup/shared/system-overview.md` | -- |
| Instruction Budget | The practical limit (~100-150 instructions) an agent can follow reliably; exceeding it degrades all instructions uniformly. | `ai-docs/decisions/ADR-029-instruction-budget-constraint.md` | Line budget |
| Layer 1-5 | The five-layer architecture: Runtime (always on), Local Context (auto-load), Skills (on demand), Playbooks (on demand), Evaluation (on demand). | `workflow/setup/shared/system-overview.md` | 5-layer system |
| Learning Loop | The feedback cycle where agent mistakes become permanent project knowledge via lesson and footgun entries. | `workflow/setup/shared/phase-1.md` | -- |
| Lesson | A documented behavioural mistake stored in category bucket files under `ai-docs/lessons/`. | `workflow/setup/shared/phase-1.md` | -- |
...
| Revert-and-Rescope | A three-step recovery tactic (Esc + restate, git revert + rescope, /clear + handoff) used when two corrections fail on the same approach. | `workflow/setup/shared/execution-loop.md` | -- |
| Router Table | An index at the end of CLAUDE.md pointing to all project resources; tools listed here get 160x more agent usage. | `workflow/setup/shared/execution-loop.md` | -- |
...
| Skill Justification Test | The gate requiring each skill to have at least one of: distinct artefact, hard workflow gate, special failure mode, or repeatable structured output. | `ai-docs/decisions/ADR-030-skill-consolidation.md` | -- |
| State Declaration | The required format (`State: [MODE] \| Goal: [one line] \| Exit: [condition]`) an agent must announce before acting. | `workflow/setup/shared/execution-loop.md` | -- |
| Stop-the-Line | A Level 2 VERIFY escalation requiring the agent to fully stop, preserve error output, and wait for human review. | `workflow/setup/shared/execution-loop.md` | Level 2 escalation |
