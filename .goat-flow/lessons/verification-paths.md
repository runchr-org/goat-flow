...
**Recurrence 2026-05-26:** A hook-rename verification run failed `stats --check` because external PR paths in `.goat-flow/footguns/quality.md` and `.goat-flow/footguns/skills.md` were backticked like repo-local files, a local anchor still searched for the old `12 setup-scope checks` wording, and `.goat-flow/lessons/external-pr-learnings.md` used fake local paths in an example. The repair kept external paths as prose, updated the local anchor to `src/cli/audit/check-goat-flow.ts` (search: `15 setup-scope checks`), and removed fake backticked paths from the lesson example. Evidence anchors: `.goat-flow/footguns/quality.md` (search: `external PR's specs/spec-open-questions-workflow`), `.goat-flow/footguns/skills.md` (search: `external specs/add-feedback-workflow-command`), `.goat-flow/lessons/external-pr-learnings.md` (search: `specific changed file, section, and grep-friendly evidence string`).
