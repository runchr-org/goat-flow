...
- Goat-flow surfaces at risk: `src/cli/audit/check-goat-flow.ts` (search: `15 setup-scope checks`) currently asserts structural presence of `.goat-flow/architecture.md`, `code-map.md`, etc. — but does not inspect their content for unresolved questions. Same applies to milestone files in `.goat-flow/plans/**`.
