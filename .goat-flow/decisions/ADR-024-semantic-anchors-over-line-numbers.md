...
Footgun and lesson entries cited code evidence using `file:line` references (e.g. `guardrails-self-test.sh:88-96`). Three independent Gemini quality reports in a single session flagged stale line numbers in footgun evidence - references that pointed at the right file but wrong code because line numbers shift on every edit. The framework's own `stats --check` validated that cited lines were in-bounds but could not detect that the content at those lines no longer matched the described evidence.
