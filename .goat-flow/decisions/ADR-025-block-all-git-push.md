...
The deny-dangerous.sh hook previously blocked only pushes to protected branches (main, master, production, deploy) and force pushes. Feature-branch pushes were allowed. This was based on the assumption that agents should be able to push to feature branches as part of a PR workflow.
...
- `deny-dangerous.sh` now blocks any `git push` command with a single pattern match (check 3).
