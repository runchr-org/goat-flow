...
- `src/dashboard/dashboard-terminal.ts` (search: `AWAITING_INPUT_TAIL_VISIBLE_RANGE`) is the canonical fix after three rounds of per-runner spinner-glyph patches proved the chunk-level classifier is structurally fragile. The message handler now defers to the merged tail: when `dashboardNextAwaitingInputState` returns false, the badge/timer only clears if the prompt is also no longer in the normalized last 1500 visible chars derived from `outputTail`. Unknown chunks (new spinner glyphs, mode toggles, OSC updates, future runner versions) can no longer kill the badge while the prompt is still visible. Defense-in-depth: the spinner-glyph branch at (search: `spinner-glyph frame`) still classifies bare `●` / `◦` / braille frames as transient so the chunk-level result also stays true in the common case. Pattern reference: `.goat-flow/patterns/architecture.md` (search: `Tail-driven state for streaming TUI heuristics`). Lesson reference: `.goat-flow/lessons/design-decisions.md` (search: `Three rounds of the same fix shape`).
