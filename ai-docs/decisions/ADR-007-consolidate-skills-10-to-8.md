...
- **goat-onboard** produced an orientation document - but goat-debug already reads architecture, footguns, and router tables. Adding an "onboard mode" to debug avoids a separate skill for what is essentially a scoped investigation.
...
- **goat-context** (renamed from goat-resume) reconstructed session state from handoff files - but modern agents (Claude Code, Codex) handle context resumption natively. The skill added no value beyond what `/compact` and `.goat-flow/tasks/handoff.md` already provide.
...
- goat-onboard → goat-debug (Onboard Mode): systematic codebase mapping for new contributors or agents
...
**Additions:**
- goat-investigate → goat-debug (Investigate Mode): deep codebase investigation with progressive depth reading
- goat-simplify → goat-review (Simplify Mode): readability improvement with semantics-preserving constraint
- goat-refactor → goat-plan (Refactor Planning Mode): both-sides-first reading, grep-after-every-rename, blast radius declaration

**Canonical set:** goat-debug, goat-plan, goat-review, goat-security, goat-test (+ goat dispatcher).
...
- Modes within goat-review, goat-debug, and goat-plan are triggered by the user's intent, not separate slash commands - this requires the skill to detect context (e.g., "review my instruction files" triggers Instruction Review Mode)
