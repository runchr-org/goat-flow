...
- **Workspace Boundary harness concern** - `audit --harness` expands from 16 checks across 5 concerns to 18 checks across 6 concerns. The new `workspace_boundary` concern adds `boundary-guidance-present` and `boundary-path-separation`, with CLI rendering, dashboard Home/Quality surfaces, factual-claim counts, and harness docs updated to match.
