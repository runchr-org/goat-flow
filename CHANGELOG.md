...
Dashboard Skills cleanup, runner launch reliability fixes, and skill evaluator shape detection.
...
- **Skill evaluator shape detection** - Skill-quality evaluation now keeps `subtype` as the applied scoring profile while adding `detectedShape`, `shapeConfidence`, and `shapeMismatch` for cases such as "packaged as skill, reads like playbook." Fallback-only subtype matches no longer report full confidence, browser/MCP commands count as tool dependencies requiring availability/fallback handling, and dashboard, prompt, API, and docs surfaces expose mismatch evidence without changing existing subtype consumers. The rubric also stays portable for uploaded or non-goat-flow skills by crediting explicit context, prerequisites, gates, and evidence rules instead of requiring goat-flow preamble inheritance. (`src/cli/quality/skill-quality.ts`, `src/cli/quality/quality-config.ts`, `src/cli/prompt/compose-quality.ts`, `src/dashboard/app.ts`, `src/dashboard/views/skills.html`, `src/dashboard/globals.d.ts`, `docs/skill-quality-config.md`, `test/unit/skill-quality.test.ts`, `test/unit/skill-quality-prompt.test.ts`, `test/integration/dashboard-server.test.ts`)
