...
- **`goat-flow audit` no longer executes the target's hook code by default** - The deny-mechanism runtime smoke (which runs the audited checkout's own configured launcher and managed hook script) is now opt-in via `--deny-runtime-smoke`, so a plain audit runs static deny checks only and is safe to point at an untrusted checkout. Pass `--deny-runtime-smoke` to restore the previous runtime launcher/glue proof, e.g. when auditing your own project in CI.
