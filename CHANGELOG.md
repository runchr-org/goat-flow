...
- **Skill quality calibration (M07)** - The deterministic skill-quality scorer is now calibrated against the in-tree 12-artifact corpus. Skills are scored against a *composed* runtime surface (SKILL.md + skill-preamble.md + skill-conventions.md + local references) rather than SKILL.md in isolation, so gate vocabulary (`BLOCKING GATE`, `Proof Gate`, `CHECKPOINT`) inherited from the preamble is now visible to gate/evidence/tool scorers. Subtype detection added (`workflow`, `dispatcher`, `report`, `playbook`, `index`, `meta`) with per-subtype profile maxes (workflow=100, dispatcher=70, report=85, playbook=80, index=60, meta=50). Recommendations gain a hard-gate rule (any 0-metric forces `needs-human-review`), retire-before-demote ordering, and `consider-reclassifying` for high-structure-but-low-confidence cases. Snapshot regression fixture at `test/fixtures/skill-quality/expected-scores.json` locks the cross-corpus result. Discovery hardening: walk roots include `.agents/skills`, `.github/skills`, `workflow/skills`; sub-references read from the `references/` subdirectory; symlinks skipped; per-file 256 KiB cap. Frontend race-guard, project-switch reload, and table-pipe escape on the prompt composer all in. (`src/cli/quality/skill-quality.ts`, `src/cli/prompt/compose-quality.ts`, `src/dashboard/app.ts`, `src/dashboard/views/skills.html`, `src/dashboard/globals.d.ts`, `test/fixtures/skill-quality/expected-scores.json`, `test/unit/skill-quality.test.ts`, `test/unit/skill-quality-prompt.test.ts`)
- **Dashboard "Create new skill" modal (M08c §5)** - Skills page now has a `+ Create new skill` button that opens a modal with two tabs (From description / From draft). The modal calls two new authorized endpoints — `POST /api/quality/candidacy` (token-gated, Origin-checked, validates payload structure and 32 KiB / 256 KiB caps on text/content) and `POST /api/quality/scaffold` (same gates, plus mandatory `confirm: true` flag — refuses to write without it). Description mode wraps the candidacy engine; draft mode treats the pasted markdown as the candidacy input. When the recommendation is `skill` or `reference (playbook)`, a "Scaffold and write" button writes the file under the canonical path, surfaces the initial rubric score, and refreshes the artifact inventory; non-skill recommendations show reasoning + next steps without scaffolding. Nine new integration tests in `test/integration/dashboard-server.test.ts` cover happy-path scaffolding, the `confirm: true` requirement, payload validation, and 405 on non-POST. Browser verification pending. (`src/cli/server/dashboard-routes.ts`, `src/cli/server/dashboard.ts`, `src/cli/server/decoders.ts`, `src/dashboard/views/skills.html`, `src/dashboard/app.ts`, `src/dashboard/globals.d.ts`, `test/integration/dashboard-server.test.ts`)
- **`goat-flow skill new` authoring CLI (M08b §4)** - New top-level `skill` command with `new` subcommand wraps the candidacy engine to scaffold workflow / dispatcher / report skills and reference playbooks from either a description (`goat-flow skill new "I want a workflow that …" --name <kebab-case> --yes`), an existing draft (`goat-flow skill new --draft path/to/draft.md`), or an interactive prompt (`goat-flow skill new --interactive`). Description mode that doesn't match a skill or playbook intent prints the candidacy reasoning and stops — never silently scaffolds the wrong shape. Draft mode is non-destructive: it validates classification and suggests a `mv` when the file is in the wrong location, never writing or moving on its own. Templates score ≥ 70% structurally as starting points; post-scaffold the rubric is run automatically and the score is surfaced. Inline scaffold templates avoid template-file packaging concerns. (`src/cli/skill-author.ts`, `src/cli/cli.ts`, `test/integration/skill-author.test.ts`)
- **Configurable skill-quality rubric and candidacy check (M08a + M08b §3)** - Quality engine now reads from a `QualityConfig` (defaults match M07 exactly; consumer projects override via `.goat-flow/config.yaml` `quality` block). Custom subtype profiles, gate vocabulary, walk roots, composition recipe, and fixture paths can all be declared without forking goat-flow. New classification engine returns confidence + alternatives so the dashboard distinguishes "well-built artifact" from "correctly classified artifact" — the `consider-reclassifying` recommendation fires when structure is high but subtype confidence is below 70%. New `goat-flow quality candidacy` CLI command (and underlying `runCandidacyCheck` engine) accepts either `--draft <path>` or a description string and recommends what kind of artifact to create — skill, reference, instruction-file rule, learning-loop entry, CLI command, or "don't create." Description-mode never auto-defaults to skill; ambiguous inputs return `do-not-create` with low confidence. Dashboard surfaces the classification card and reclassification banner; `goat-flow skill new` interactive scaffold and dashboard authoring modal remain pending (M08b §4 / M08c). (`src/cli/quality/quality-config.ts`, `src/cli/quality/candidacy.ts`, `src/cli/quality/skill-quality.ts`, `src/cli/cli.ts`, `src/cli/config/types.ts`, `src/cli/config/reader.ts`, `src/dashboard/views/skills.html`, `src/dashboard/globals.d.ts`, `test/unit/quality-config.test.ts`, `test/unit/candidacy.test.ts`)
...
- **Dashboard Skills page** - New top-level `Skills` nav tab hosts a per-artifact skill-quality workbench that classifies each `.claude/skills/*/SKILL.md` and `.goat-flow/skill-reference/*.md` artifact across 9 deterministic structural metrics (trigger clarity, workflow completeness, gate quality, evidence/testability, cold-start, token cost, tool-deps, write-risk, skill-vs-reference fit). Each artifact gets one of six explicit recommendations: `keep-skill`, `split-skill`, `merge-or-demote`, `reference-playbook`, `retire`, or `needs-human-review`. New `/api/skill-quality/inventory` returns the artifact list; `/api/skill-quality?artifact=<id>` returns the score, metric breakdown, fit notes, and a focused review prompt that can be launched in a Runner session. The workbench was originally placed in the Quality view's left column but lives on its own page now so the wider grid renders without sticky-prompt competition. No schema changes — first version is dashboard-only deterministic output. (`src/cli/quality/skill-quality.ts`, `src/cli/server/dashboard-routes.ts`, `src/cli/prompt/compose-quality.ts`, `src/dashboard/views/skills.html`, `src/dashboard/index.html`, `src/dashboard/app.ts`, `src/dashboard/globals.d.ts`, `workflow/manifest.json`, `test/unit/skill-quality.test.ts`, `test/integration/dashboard-server.test.ts`)
- **Terminal image drag-and-drop** - Workspace terminal pane now accepts dragged image files (`.png`, `.jpg`, `.jpeg`, `.webp`, `.gif`) up to 10 MiB each, max 5 per drop, max 25 MiB per request. New authorized endpoint `POST /api/terminal/:id/upload-image` validates session ownership, sanitizes filenames, verifies image magic bytes (rejects content-extension mismatches like text-as-`.png`), and saves files under `<targetPath>/.goat-flow/logs/uploads/<sessionId>/` (gitignored). On success, the dashboard pastes a plain-text attachment note (`Attached image: <relpath>`) into the active PTY via the existing bracketed-paste path; rejected files surface inline toast errors with reasons. WebSocket PTY contract stays text-only — uploads never travel through the input stream. (`src/cli/server/terminal-uploads.ts`, `src/cli/server/dashboard-terminal.ts`, `src/cli/server/dashboard-routes.ts`, `src/cli/server/dashboard.ts`, `src/cli/server/decoders.ts`, `src/dashboard/app.ts`, `src/dashboard/dashboard-readers.ts`, `src/dashboard/views/workspace.html`, `test/unit/terminal-uploads.test.ts`, `test/integration/dashboard-server.test.ts`)
- **Codex permissions and feature-flag correction** - Codex CLI 0.129.0 still uses `[features].codex_hooks = true` as the live flag (the prior 1.5.1 migration to `[features].hooks = true` was based on outdated docs). Reverted the template/installer back to `codex_hooks` and inverted `migrate_codex_hooks_feature_flag` so existing installs migrate the other direction. Codex template now also writes a TOML permission profile (`default_permissions = "goat-flow"`) that denies `.env*`, `.envrc`, `secrets/`, `.ssh/`, `.aws/`, `.docker/config.json`, `.gnupg/`, `.npmrc`, `.pypirc`, and key/credential families at the filesystem layer. Audit `agent-settings` and harness `deny-covers-secrets` now classify Codex against permission-profile file-read coverage (not Claude-style `Read()` patterns). Lesson recorded: Claude-style settings vocabulary does not transfer 1:1 to Codex; verify against current vendor docs before mapping. (`workflow/hooks/agent-config/codex.toml`, `.codex/config.toml`, `workflow/install-goat-flow.sh`, `src/cli/audit/check-agent-setup.ts`, `src/cli/audit/harness/check-constraints.ts`, `src/cli/facts/agent/settings.ts`, `src/cli/facts/agent/hooks.ts`, `.goat-flow/footguns/hooks.md`, `.goat-flow/lessons/agent-behavior-trust.md`, all six `deny-dangerous.sh` mirrors, `docs/audit-checks.md`, `docs/deny-dangerous.md`, `docs/harness-audit.md`, `workflow/hooks/README.md`, `workflow/setup/agents/codex.md`, `workflow/manifest.json`)
