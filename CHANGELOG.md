...
- **Fail-closed hook startup and verification** - Missing or unreadable `.goat-flow/hook-lib/` now denies with an agent-shaped failure instead of falling through to exit 127. Audit and preflight check the dispatcher, the shared store, and the exact configured hook commands for Claude, Codex, Antigravity, and Copilot, so stale paths, lost executable bits, and command-shape regressions fail before an agent session sees them.
...
- **Regression coverage** - Added tests for Home readiness scoring, Setup target card scoring, full-scope setup prompt behavior, outdated prompt issue details, and `/api/setup` self-test avoidance. (`test/integration/dashboard-server.test.ts`, `test/unit/audit-command/main.test.ts`, `test/unit/preset-prompts.test.ts`)
