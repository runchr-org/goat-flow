...
Antigravity replaces Gemini CLI as the third actively-supported agent; report-only skills require a four-class proof tag on every finding; the Codex installer migrates filesystem permission shapes that previously broke startup; and the dashboard's awaiting-input badge is rearchitected to clear only on user input after five rounds of output-driven heuristics failed. Two new on-demand playbooks (observability and code comments) ship, and the deny hook treats GitHub CLI as read-only by default.
...
- **Proof class required on every finding** - `goat-critique`, `goat-qa`, and `goat-security` now require one of `RUNTIME | CONTRACT-GREP | STATIC | NOT-REPRODUCED` on every sub-agent finding, validated finding, recommended change, and gap-analysis output. The existing OBSERVED/INFERRED/UNVERIFIED evidence-quality tag is kept; proof class names *how* the claim was established, not *how confident*. Output templates, rubric examples, meta-audit criteria, and report-only skill hardening contracts updated; new contract tests pin the requirement. The goat-qa skill also got a clarity pass on description, "when to use" guidance, output expectations, and coverage vocabulary, with proof-class counts surfaced in the Verification Integrity section.
- **Codex install: filesystem permissions migrated in place** - `goat-flow install . --agent codex` now migrates the legacy `:project_roots` anchor to `:workspace_roots` and rewrites invalid glob `none` entries (e.g. `"*.pem" = "none"`, `"**/*.key" = "none"`) into the canonical block Codex 0.131 accepts (exact paths plus trailing `/**` subtrees). A post-install validator aborts the install if any unsupported shape would still crash Codex at startup. Migration matcher and validator share one `isInvalidNoneKey` predicate so a valid `"secrets/**" = "none"` deny is never false-flagged; invalid globs inside inline `":workspace_roots" = { ... }` tables are caught by migration, not just rejected later; the validator scans only inside `[permissions.*.filesystem*]` sections so unrelated tables don't block install; legacy-anchor detection uses the quoted form `":project_roots"` so comments referencing the legacy name don't trigger.
- **Dashboard awaiting-input badge: asymmetric trust (Round 6)** - the badge is now SET from runner output but CLEARED only by input-side authoritative signals: `term.onData` keystrokes, `dashboardSendToTerminalSession` programmatic sends, Ctrl+V paste through the custom key handler, and session lifecycle events (exit, terminating error, detach-as-end). Five prior rounds tried to clear the badge from output (glyph allowlists for `●`/`◦`/braille/circles, normalized tail-end heuristics, raw-byte slices that preserve OSC titles) - each round passed its tests but a new runner pattern always defeated it. The trade-off is explicit: a stuck badge after the runner is answered out-of-band (e.g. via Claude's remote control) is preferable to a badge that never fires at all. New PTY fixtures cover Claude trust/bash-approval/running/startup, Codex startup/running, and Copilot startup/running; the architecture is captured in `.goat-flow/footguns/dashboard.md` and `.goat-flow/patterns/architecture.md`.
- **Observability playbook** - new on-demand playbook (`.goat-flow/skill-playbooks/observability.md`) for instrumenting application code with logs, metrics, span events, and trace context. OpenTelemetry-shaped (instrument kinds, severity numbers, semantic attribute names, trace correlation rules), but the discipline applies to any backend that ingests structured logs and metrics. Loaded when adding the first signal to a service, reviewing an instrumentation diff, or fixing a postmortem instrumentation gap.
- **Code-comments playbook** - new on-demand playbook (`.goat-flow/skill-playbooks/code-comments.md`) covering WHEN and WHY to write a comment, docstring, or annotation. Portable across TypeScript, Python, Go, Rust, and shell; defers to each language's docstring syntax (JSDoc, PEP 257, godoc, rustdoc) and owns only the keep/rewrite/delete decision.
- **GitHub CLI read-only by default (ADR-028)** - the deny hook now blocks `gh` write paths: issue/PR comments, releases, workflow runs, repo edits, labels, gists, secrets, variables, keys, auth changes, extensions, codespaces, project mutations, cache deletion, and `gh api` write methods or body-field POST forms. Read-only `gh issue view`, `gh pr view/diff/checks`, `gh search`, `gh repo view`, and `gh api --method GET/HEAD` remain available for review and CI evidence gathering. Driven by a real 2026-05-20 incident where a coding agent interpreted forwarded text as authorization to post a GitHub issue comment.
- **npm publish authentication hardening** - `scripts/npm-publish.sh` documents the granular access token / 2FA bypass setup at the top of the file, accepts `NPM_TOKEN` or `NODE_AUTH_TOKEN` from the environment, writes the token to a temporary `.npmrc` that is removed on exit, and emits clearer messaging so a stalled OTP step is distinguishable from a real publish error.
- **Dependency-update workflow** - `scripts/dependency-update.sh` adds an npm `overrides` review step and clearer output so transitive-dep version drift is visible before commit.
- **Tool playbook index** - instruction files (CLAUDE.md, AGENTS.md, .github/copilot-instructions.md) now point at `.goat-flow/skill-playbooks/README.md` as the single capability-playbook index instead of listing each playbook individually. The "read BEFORE declaring a tool unavailable" rule still names browser-use and page-capture explicitly so the parity check passes.
- **GitHub CLI examples use `owner/repo`** - test fixtures, docs, and ADR-028 narrative now use the neutral `owner/repo` placeholder instead of project-specific repo names in `gh` command examples.
...
- **Historical artefacts preserved** - `CHANGELOG.md`, manifest snapshots `v1.1.0.json`–`v1.7.0.json`, and the `gemini-startup.txt`/`gemini-running.txt` PTY captures (used by runner-agnostic awaiting-input parser tests) remain untouched. New manifest snapshot `v1.8.0.json` frozen.
