...
Cross-rater skill quality audit, skill hardening, structural reorganisation, dispatcher rewrite, page-capture shared reference, project-stack detection improvements, and v1.3.3 features rolled in.

- **Cross-rater skill quality audit** - Three independent AI raters (Claude, Codex, Gemini) scored all 6 skills on a 10-axis rubric. Average improved from 76/100 (main) to 87/100 (feat). Arithmetic and factual accuracy verified across raters; Codex was most accurate (8/8 new features found).
- **Excuse/Reality tables for 3 skills** - goat-critique, goat-plan, and goat-qa each gained 4-entry tables grounded in real incidents (ADR-021, lessons/agent-behavior-trust, lessons/verification, lessons/design-decisions). All 6 skills now have E/R tables.
- **Proof architecture in goat-review** - Proof Capsule with 4-class Proof Classification (RUNTIME/CONTRACT-GREP/STATIC/NOT-REPRODUCED), Review Scope Snapshot, Refutation Ledger for audit trail of dismissed suspicions, Blast Radius Rule requiring call-site grep for contract changes, Ship Verdict with Consequence Gate, Step 0.5 Intent Reconstruction, and Pass 3 Cross-Model Refuter (opt-in). Review Integrity expanded to 12 degradation flags.
- **Integrity blocks** - goat-debug gained Debug Integrity (7 fields) and D1.5 Minimise phase with 2 new constraints. goat-security gained Security Assessment Integrity (7 fields), Phase 5.5 Exploit Chaining, and Persist Gate.
- **goat-critique improvements** - Phase 5.5 meta-audit against 10-point rubric, Phase 5.6 outcome capture, integration hooks routing findings to goat-plan/goat-debug/implementation.
- **goat-qa improvements** - A1.5 Scope-Size Gate for audit mode, 10-field Verification Integrity with OBSERVED/INFERRED/UNVERIFIED coverage-claim tagging, Coverage Depth vocabulary (NONE/STRUCTURAL/PARTIAL-BEHAVIOURAL/BEHAVIOURAL), Intent vs Reality Diff table.
- **Patterns directory** - Restructured `.goat-flow/patterns.md` into `.goat-flow/patterns/` with 5 categorised bucket files (verification, refactoring, multi-agent, workflow, architecture) matching the footguns/lessons pattern. Updated 30+ cross-references across instruction files, CLI code, docs, setup templates, test fixtures, manifest, and installer.
- **Skill-specific reference packs relocated** - Moved 5 skill-specific reference files from shared `.goat-flow/skill-reference/` into per-skill `references/` subdirectories (goat-critique/2, goat-review/2, goat-plan/1), matching the existing goat-security pattern. Shared directory now holds only truly shared files.
- **DDT layer removal (ADR-027)** - Removed `.goat-flow/skill-reference/ddt-layer/` (per-language static analysis packs). Framework-shipped per-language instructions don't scale across project toolchains. goat-plan now says "language-appropriate static analysis" and lets agents detect the toolchain.
- **Goat dispatcher rewrite** - Rewrote goat/SKILL.md from 585w to 549w (under revised ≤550w cap). Added Route Snapshot output contract, multi-intent decomposition protocol, GATHER checklist, verification routing distinction (ADR-018), ambiguity examples, explicit-invocation-first rule, no-inspect constraint, degraded-routing fallback. ADR-023 amended.
- **Page-capture shared reference** - New `.goat-flow/skill-reference/page-capture.md` for batch page capture via Playwright MCP. Complements `browser-use.md` (single observation) with structured workflow for visiting N pages, screenshotting, and emitting MD records. Wired into installer, drift checks, preflight sync, and content-quality audit.
- **ISSUE.md in goat-plan** - Mode 4 now auto-creates ISSUE.md alongside milestone files (Why/What/How format). Phase 4 close-out revises it. Format reference at `references/issue-format.md`.
- **Project-stack detection improvements** - Separated test frameworks (Cypress, Playwright, Jest, Vitest, Mocha) from app frameworks in `project-stack-data.json`. Added 6 Node frameworks (Fastify, Hono, NestJS, Remix, Astro, Solid), 3 language signals (Dart, Kotlin, Elixir), 3 code-gen signals (GraphQL codegen, Prisma, Drizzle), 6 deploy signals (Netlify, Cloudflare, Render, Pulumi, Kubernetes, AWS CDK), 10 setup framework markers (Laravel, Symfony, Django, FastAPI, Rails, Spring Boot, Remix, Astro, Phoenix, Flutter), and 4 formatter entries (Kotlin, C#, Swift, Dart). Fixed Jinja false-positive on `**/*.html`. Added Docker Compose V2 filenames (`compose.yaml`).
- **Configurable goat-review local PR base** - `.goat-flow/config.yaml` can now define `skills.goat-review.local_pr_base` so local PR reviews compare against a project integration branch before falling back to remote default-branch discovery.
- **Workspace sidebar collapse** - Dashboard Workspace sessions rail has a collapse/expand footer control with persisted state, accessible labels, smooth grid-column transition, and compact collapsed rail.
- **Harness aggregate boundary scoring** - Aggregate `audit --harness` now requires every audited configured agent to have path-agnostic workspace boundary guidance instead of passing when any one agent has it.
- **Dashboard harness percentage parity** - Dashboard report readers preserve harness check `type` from `/api/audit`, so Home, Setup, and Quality per-agent percentages exclude metric checks; healthy agents display 100% instead of 94%.
- **Path-only task intake guard** - goat-plan and dispatcher instructions treat bare or ambiguous `.goat-flow/tasks/...` references as read-only orientation context, blocking mutations until the user gives an explicit action verb.
- **Manifest snapshots** - Backfilled v1.3.1 and v1.3.2 manifest snapshots.
- **Release propagation** - Package/config/manifest, skill mirrors, shared references, security reference packs, fixtures, hooks, and instruction files bumped to 1.4.0.
