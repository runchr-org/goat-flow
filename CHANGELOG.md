...
- **Codex terminal startup hardening** - Codex permission profiles now avoid 0.131-rejected filename globs, keep workspace-root denies in Codex 0.131's accepted `:workspace_roots` inline-table shape, audit detection follows the exact-path/subtree shape Codex accepts, and dashboard launch prompts are suppressed when a runner fails before reaching its composer.
