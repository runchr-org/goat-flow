...
- **Gruff hook diagnostics are safer in monorepos** - `gruff-code-quality` stays scoped to the edit: payloads naming only non-analyzable paths (docs/config edits) skip instead of sweeping git-changed files, and the git fallback covers only payloads carrying no paths at all. It also honours ignored files as explicit skipped output, finds configured analyzer binaries in non-standard layouts, reports timeout/config problems, and preserves changed-line scoping where analyzers support it.
...
- **PR review follow-ups** - Registered gruff hook timeouts rise to 90s (Claude/Antigravity/Copilot) so the script's own 60s analyzer timeout can report diagnostics before the runner kills the wrapper; the `doc-paths-resolve` audit check now fails when `.goat-flow/architecture.md` is missing instead of letting resolved-path counts balance the finding away; dashboard skill-quality inventory errors and skill-evaluator verdicts are staleness-guarded across project switches; the dashboard test fake clock clamps zero-delay intervals so `tick()` cannot spin.
