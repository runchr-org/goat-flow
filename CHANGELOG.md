...
- **Quality-Report Follow-Ups (M17, slot reused 2026-04-19)** - Four 2026-04-19 agent-generated quality reports surfaced real defects the audit was passing: a CRITICAL Bash secret-read gap (runtime-probe caught it; three static-only reports missed it), a dead `Notification`/`compact` hook the harness reported as installed, 17-vs-16 harness-check doc drift, and a `goat-qa` audit-mode output-contract mismatch. M17 closes them in 14 slices, expanded mid-flight to absorb a TypeScript-safety critique (T1-T7). Core fixes:
...
- **In Progress (1.2.0 plan, not yet shipped)** - M08 (skill TDD + rationalization tables), M09 (structured learning loop), M10 (progressive-disclosure retrieval), M11 (back-fill provenance on existing audit checks), M12 (multi-agent support matrix), M13 (quality persistence + score trend — partial, quality validate + dashboard history view shipped), M14 (skill-reference plumbing), M15 (goat-security core), M16 (quality dashboard surface — partial, schema + API shipped; view-side finding rendering deferred). Tracked in `.goat-flow/tasks/1.2.0/`.
