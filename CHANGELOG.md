...
- **Dashboard** - Dedicated Prompts page with category filters, Sessions rail (cap=7), quality-history view. Server split into `dashboard.ts` + `dashboard-routes.ts` + `dashboard-terminal.ts` + `setup-detect.ts` + `dashboard-assets.ts`. Presets moved to `src/dashboard/preset-prompts.json`, stack tables to `workflow/project-stack-data.json`. State persistence in `.goat-flow/dashboard-state.json` (was port-scoped localStorage).
