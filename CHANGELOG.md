...
`.env.example` read-only handling, richer dashboard presets, dashboard audit caching, reporting-only quality prompts, active-plan local-state handling, goat-critique contract tightening, prompt-loading polish, shared-reference doc cleanup, widened dashboard lint coverage, and goat-qa gate clarification.
...
- **Dashboard audit cache** - Packaged dashboard runs now cache full harness audit results in `.goat-flow/audit-cache.json`, keyed by package version and project config version. Normal dashboard loads can reuse the cache, while visible Refresh actions pass `fresh=true`; the UI marks cached results and preserves the cached timestamp.
...
- **Landing page** - Added `docs/goat-flow-landing.html`, a standalone product page with harness-engineering positioning, system/skills/hooks/learning-loop sections, copyable install commands, and links to docs/GitHub.
...
- **Quality reporting mode** - Quality prompts now use "reporting-only" instead of strict read-only language, explicitly allowing gitignored validation/reporting/local-state artifacts while still forbidding tracked-file writes, patches, and implementation. Unit tests and dashboard quality copy were updated to match.
- **Active plan marker semantics** - `/goat` and `/goat-plan` now treat `.goat-flow/tasks/.active` as an advisory local pointer, not a setup invariant. Missing or stale markers fall back to listing non-archived task dirs and asking which plan is current; ADR-017, glossary, code-map, and the quality prompt document the same contract.
- **goat-critique contract** - The critique skill now makes explicit invocation binding, tightens skill-chained retrieval/rubric handling, scans Agent C output for context leaks before ranking, names per-agent tool budgets, and trims duplicated constraint/output prose while preserving the full delegated 5-phase flow.
...
- **Quality finding cleanup** - Setup execution-loop reference now matches installed doctrine: skill Step 0 replaces READ while SCOPE still applies before writes. Active footgun evidence was corrected for current secret-read hook behavior and stale learning-loop retrieval anchors were removed.
