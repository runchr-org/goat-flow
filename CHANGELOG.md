...
Release-readiness hardening for decision records, dashboard audit performance, workspace-boundary checks, and quality-report evidence. PR #27 spans 145 files with 3,245 insertions and 452 deletions.
...
- **Workspace boundary guidance moved to Context** - The `boundary-guidance-present` check (whether instruction files distinguish the controlling workspace from the selected target) is now a Context advisory check instead of a separate concern. The `boundary-path-separation` metric is removed. The harness model stays at 5 concerns (context, constraints, verification, recovery, feedback loop) with 17 checks. Dashboard agent scores now exclude metric checks from the percentage, matching how concern scores already worked.
- **Harness scoring honesty** - Metric checks no longer inflate concern score denominators or numerators, and the test-runner metric now reports `fail` when no configured test command exists while still remaining informational.
