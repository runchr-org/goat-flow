...
function runStopLint(cwd: string): { status: number; stdout: string; stderr: string } {
  try {
...
function runStopLintSplit(cwd: string): { status: number; stdout: string; stderr: string } {
...
describe('stop-lint.sh hook source integrity', () => {
  it('does NOT use || true to SUPPRESS validation results', () => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const isValidationCmd =
        /shellcheck|eslint|tsc|phpstan/.test(line);
