...
// 2.2.3 (Post-turn hook does not swallow failures) removed - check deleted, AP6 covers this.
    ...
    const ctx = createMockContext({
      agentFacts: {
        instruction: {
          ...
          lineCount: 5,
          sections: new Map([["log", "Update lessons when tripped."]]),
        },
      },
    });
    const result = runSingleCheck(check, ctx);
...
describe("Check 2.4.3: Skills referenced in router", () => {
  ...
  it("fails when router uses legacy goat-* glob", () => {
    ...
    assert.ok(
      result.message.includes("goat-*") ||
        result.message.includes("dispatcher"),
    ...
    const ctx = createMockContext({
      agentFacts: {
        instruction: {
          ...
          content: "# CLAUDE.md\n\n## Router Table\n| Docs | `docs/` |\n",
          lineCount: 5,
          sections: new Map([["router table", "| Docs | `docs/` |"]]),
        },
      },
    });
    const result = runSingleCheck(check, ctx);
...
describe("Check 2.3.4: Footgun evidence", () => {
  ...
  it("fails when footguns have stale file references", () => {
    ...
    assert.ok(
      result.message.includes("missing files") ||
        result.message.includes("stale"),
  ...
  it("fails when footguns have out-of-range line references", () => {
    ...
    assert.ok(
      result.message.includes("out-of-range") ||
        result.message.includes("line"),
...
describe("Anti-pattern AP23: Overly broad deny patterns", () => {
  ...
  it("does not trigger with specific deny patterns", () => {
    ...
    assert.equal(
      result.triggered,
      false,
      `Expected not triggered: ${result.message}`,
    );
  });
  ...
  it("triggers on Bash(*git*) - too broad", () => {
    ...
    assert.equal(
      result.triggered,
      true,
      `Expected triggered: ${result.message}`,
    );
...
describe("Anti-pattern AP13: Stale instruction refs", () => {
  ...
  it("does not trigger when paths reference real files", () => {
    ...
    assert.equal(
      result.triggered,
      false,
      `Should not trigger for existing path: ${result.message}`,
    );
  });
...
// AP17 (Dangling file references in skills) removed.
...
describe("Check 1.1.2: Instruction file under line target", () => {
  ...
  it("partial when 121-150 lines", () => {
    const ctx = createMockContext({
      agentFacts: { instruction: { lineCount: 135 } },
    });
    const result = runSingleCheck(check, ctx);
  ...
  it("fails when over 150 lines", () => {
    const ctx = createMockContext({
      agentFacts: { instruction: { lineCount: 160 } },
    });
    const result = runSingleCheck(check, ctx);
      ...
      agentFacts: {
        instruction: {
          ...
          sections: new Map([
            [
              "execution loop",
              "READ CLASSIFY SCOPE ACT VERIFY LOG lessons footguns",
            ],
          ]),
        },
      },
  ...
  it("passes when askFirst section has 5+ lines with project paths", () => {
    ...
    const ctx = createMockContext({
      agentFacts: {
        ...
        askFirst: {
          exists: true,
          paths: ["docs/spec.md", "src/auth/", "workflow/skills/"],
          resolved: 3,
          unresolved: [],
        },
      },
    ...
    const ctx = createMockContext({
      agentFacts: {
        instruction: {
          content:
            "# CLAUDE.md\n\n## Autonomy Tiers\n\n**Ask First**\n- `docs/`\n- `deleted/`\n",
          sections: new Map([["autonomy tiers", "Ask First docs/ deleted/"]]),
        },
        askFirst: {
          paths: ["docs/", "deleted/"],
          resolved: 1,
...
describe("Check 1.5.2: git commit blocked", () => {
  ...
  it("passes when git commit is blocked", () => {
    const ctx = createMockContext({
      agentFacts: { deny: { gitCommitBlocked: true } },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "pass", result.message);
  });
  ...
  it("fails when git commit is not blocked", () => {
    const ctx = createMockContext({
      agentFacts: { deny: { gitCommitBlocked: false } },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "fail", result.message);
  });
...
describe("Check 1.5.3: git push blocked", () => {
  ...
  it("passes when git push is blocked", () => {
    const ctx = createMockContext({
      agentFacts: { deny: { gitPushBlocked: true } },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "pass", result.message);
  });
  ...
  it("fails when git push is not blocked", () => {
    const ctx = createMockContext({
      agentFacts: { deny: { gitPushBlocked: false } },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "fail", result.message);
  });
...
describe("Check 1.5.4: Deny hook/script exists", () => {
  ...
  it("passes when deny hook exists", () => {
    const ctx = createMockContext({
      agentFacts: { hooks: { denyExists: true } },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "pass", result.message);
  });
  ...
  it("fails when deny hook does not exist", () => {
    const ctx = createMockContext({
      agentFacts: { hooks: { denyExists: false } },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "fail", result.message);
  });
...
// ---------------------------------------------------------------
// 2.1.14 - Skills have MUST/MUST NOT constraints (has N/A gate)
// N/A when both Step 0 and constraints pass at >= 0.8 (overlap with 2.1.12)
// ---------------------------------------------------------------
describe("Check 2.1.14: Skills have MUST/MUST NOT constraints", () => {
  const check = getCheck("2.1.14");
  assert.ok(check, "Check 2.1.14 should exist");

  it("returns N/A when both Step 0 and constraints >= 0.8 (overlap with 2.1.12)", () => {
    const ctx = createMockContext(); // defaults have 5/5 for all quality signals
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "na", `2.1.14: ${result.message}`);
  });

  it("passes when constraints >= 0.8 but Step 0 < 0.8", () => {
    const ctx = createMockContext({
      agentFacts: {
        skills: { quality: { withStep0: 2, withConstraints: 5, total: 5 } },
      },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "pass", `2.1.14: ${result.message}`);
  });

  it("fails when constraints < 0.8", () => {
    const ctx = createMockContext({
      agentFacts: {
        skills: { quality: { withConstraints: 1, total: 5 } },
      },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "fail", `2.1.14: ${result.message}`);
  });
});
...
// 2.1.20 (Dispatcher installed) removed - check deleted.
...
describe("Check 2.5.3: Decisions dir ADR content", () => {
  ...
  it("passes when decisions dir has real ADR content", () => {
    const ctx = createMockContext({
      shared: {
        decisions: { dirExists: true, fileCount: 2, hasRealContent: true },
      },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "pass", result.message);
  ...
  it("fails when no decisions directory", () => {
    const ctx = createMockContext({
      shared: {
        decisions: { dirExists: false, fileCount: 0, hasRealContent: false },
      },
    });
    const result = runSingleCheck(check, ctx);
    assert.equal(result.status, "fail", result.message);
...
describe("Anti-pattern AP19: Hardcoded absolute paths", () => {
  ...
  it("triggers when hooks have absolute paths", () => {
    const ctx = createMockContext({
      agentFacts: {
        hooks: {
          absolutePathHooks: ["/home/user/project/.claude/hooks/deny.sh"],
        },
      },
    });
    const result = runSingleAntiPattern(ap, ctx);
...
describe("Anti-pattern AP20: Non-canonical skill directories", () => {
  ...
  it("triggers when stale goat-prefixed skills exist", () => {
    const ctx = createMockContext({
      agentFacts: {
        skills: {
          installedDirs: ["goat", "goat-debug", "goat-audit", "goat-reflect"],
        },
      },
    });
    const result = runSingleAntiPattern(ap, ctx);
