...
describe('Canonical path contract: no duplicate surfaces in goat-flow itself', () => {
  it('does not have both docs/footguns.md (flat) and docs/footguns/ (directory)', () => {
    ...
    if (flatFile && dirExists) {
      assert.fail(
        'Both docs/footguns.md and docs/footguns/ exist. ' +
          'Setup should use one canonical surface, not both.',
      );
    }
  });

  it('does not have both docs/lessons.md (flat) and ai/lessons/ (directory)', () => {
    const flatFile = existsSync(join(ROOT, 'docs/lessons.md'));
    const dirExists = existsSync(join(ROOT, 'ai/lessons'));
    if (flatFile && dirExists) {
      assert.fail(
        'Both docs/lessons.md and ai/lessons/ exist. ' +
          'Setup should use one canonical surface, not both.',
      );
    }
  });

  it('does not have both agent-evals/ and ai/evals/', () => {
    ...
    if (legacyDir && canonicalDir) {
      assert.fail(
        'Both agent-evals/ and ai/evals/ exist. ' +
          'Setup should migrate to one canonical surface.',
      );
    }
  });

  it('does not have both codex-evals/ and ai/evals/', () => {
    const legacyDir = existsSync(join(ROOT, 'codex-evals'));
    const canonicalDir = existsSync(join(ROOT, 'ai/evals'));
    if (legacyDir && canonicalDir) {
      assert.fail(
        'Both codex-evals/ and ai/evals/ exist. ' +
          'Setup should migrate to one canonical surface.',
      );
    }
  });
...
describe('Setup templates mention migration behavior', () => {
  const setupSharedDir = join(ROOT, 'setup/shared');

  it('setup/shared directory exists', () => {
  ...
  it('setup templates do not hardcode both old and new eval paths', () => {
    ...
    for (const file of sharedFiles) {
      ...
      const hasAiEvals = content.includes('ai/evals/');
      if (hasAgentEvals && hasAiEvals) {
        assert.fail(
          `${file} references both agent-evals/ and ai/evals/. ` +
  ...
  it('setup templates do not hardcode both flat footguns and directory footguns', () => {
    ...
    for (const file of sharedFiles) {
      ...
      const hasDirFootguns = content.includes('docs/footguns/');
      if (hasFlatFootguns && hasDirFootguns) {
        assert.fail(
          `${file} references both docs/footguns.md and docs/footguns/. ` +
...
describe('Fixture passing-minimal uses canonical paths only', () => {
  ...
  it('does not have duplicate lesson surfaces', () => {
    ...
    assert.ok(
      !flatFile,
      'passing-minimal should not have docs/lessons.md (flat file) — use ai/lessons/ only',
    );
  });
  ...
  it('does not have legacy eval path', () => {
    ...
    assert.ok(
      !legacyDir,
      'passing-minimal should not have agent-evals/ — use ai/evals/ only',
    );
  });
  ...
  it('does not have duplicate footgun surfaces', () => {
    ...
    assert.ok(
      !flatFile,
      'passing-minimal should not have docs/footguns.md (flat file) — use docs/footguns/ (dir) only',
    );
  });
});
