 ...
 */
const HOOK_EVENT_SCHEMAS: Record<
  string,
  { requiredFields: string[]; description: string }
> = {
  PreToolUse: {
    requiredFields: ["tool_name", "tool_input"],
    description:
      "Fires before a tool is executed. tool_input contains tool-specific fields (e.g., .command for Bash, .file_path for Write).",
  },
  Stop: {
    requiredFields: [],
    description:
      "Fires after every agent turn. No structured stdin - check git diff for changed files.",
  },
  Notification: {
    requiredFields: [],
    description:
      "Fires on events like compaction. Matcher filters by event type.",
  },
};
  ...
  for (const [event, schema] of Object.entries(HOOK_EVENT_SCHEMAS)) {
    it(`${event} schema has description and field list`, () => {
      ...
      assert.ok(
        ...
        `${event}: requiredFields should be an array`,
      );
    });
  }
});
