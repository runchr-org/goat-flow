...
function runScan(projectPath: string, agent = "claude"): string {
  return execSync(`node "${CLI_PATH}" scan "${projectPath}" --agent ${agent}`, {
    encoding: "utf8",
    timeout: 15000,
  });
}
...
describe("Setup output format", () => {
  ...
  it("produces valid markdown with a heading", () => {
    ...
    assert.ok(
      output.startsWith("#"),
      "Setup output should start with a heading",
    );
...
describe("Scan output basic sanity", () => {
  it("scan produces output with check results", () => {
    ...
    assert.ok(
      hasTierOrCheck,
      "Scan output should include tier or check information",
    );
  });
});
