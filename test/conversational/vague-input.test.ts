...
function extractStep0(skillName: string): string {
  ...
  assert.ok(
    step0Match,
    `${skillName}/SKILL.md must have a Step 0 section`,
  );
  return step0Match[1];
}
...
function hasQuestionPattern(step0: string, topic: string): boolean {
  ...
  const aliases: Record<string, string[]> = {
    symptom: ['symptom', 'error', 'what happened', 'what\'s wrong', 'behaviour', 'behavior'],
    ...
    'what problem': ['problem', 'what are we', 'what are you', 'goal', 'building'],
    'who affected': ['affected', 'who', 'users', 'stakeholders'],
    'done criteria': ['done', 'success', 'criteria', 'definition', 'complete', 'exit'],
    ...
    'what changed': ['what changed', 'what to test', 'change', 'diff', 'recent'],
    ...
    complexity: ['complexity', 'hotfix', 'standard', 'system', 'infrastructure'],
