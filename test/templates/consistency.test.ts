...
const SETUP_SHARED_DIR = join(ROOT, 'setup/shared');
...
describe('Execution loop: dynamic read budgets', () => {
  ...
  it('exists', () => {
    assert.ok(existsSync(execLoopPath), 'setup/shared/execution-loop.md should exist');
  });
...
// ---------------------------------------------------------------
// 8. All setup/shared/*.md files reference .goat-flow/ paths consistently
// ---------------------------------------------------------------
describe('setup/shared/*.md .goat-flow/ path consistency', () => {
  const sharedFiles = readdirSync(SETUP_SHARED_DIR)
    .filter((f) => f.endsWith('.md'));

  assert.ok(sharedFiles.length > 0, 'Should find setup/shared/*.md files');
...
// ---------------------------------------------------------------
// 9. No setup/shared/*.md file references deleted skill names
// ---------------------------------------------------------------
  ...
  for (const file of sharedFiles) {
    ...
    for (const deleted of DELETED_SKILLS) {
      it(`${file} does not reference deleted skill ${deleted}`, () => {
        assert.ok(
          !content.includes(deleted),
          `setup/shared/${file} still references deleted skill "${deleted}"`,
        );
      });
    }
  }
...
// ---------------------------------------------------------------
// 12. setup/shared/*.md all reference .goat-flow/logs/sessions/ consistently
// ---------------------------------------------------------------
